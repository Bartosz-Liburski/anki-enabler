...
Roadmap slice S-02, the north star. Turn a stored screenshot into up to 15 Q/A flashcards
oriented learned → known through one Claude vision call with structured output, then let the user
review the set on a single screen and discard the weak cards. This is the slice that proves — or
disproves — the product's core assumption, that ≥ 75% of generated cards are worth keeping
(`prd.md:36`).
...
After uploading a screenshot the user lands on that source's page, clicks generate, and gets a
capped card set with the front in the language being learned and the back in the language they
know. They toggle discard per card and save once; the decisions survive a reload. Re-generating
...
| Card cap | 15, enforced in prompt + `max_tokens` + server-side | Claude ignores array constraints and the SDK strips them silently, so a schema cap bills for 30 cards then throws. | Research |
...
| 3. Eval harness | Keep-rate per model and effort against labelled fixtures | Blocks on user-supplied screenshots; may reveal the bar is unreachable |
...
**Prerequisites:** S-01 merged; an Anthropic API key; ~10 real screenshots labelled by hand
(including one with an in-source translation, one without, and one that should yield zero cards);
Fluid Compute confirmed enabled on the Vercel project.
